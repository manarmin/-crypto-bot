...
def get_target_symbols():
    """استخراج ارزهای ترند کوین‌گکو + ۲۰۰ ارز پرحجم بایننس"""
    ranked_symbols = []
    binance_usdt_pairs = {}
    ...
    # 2️⃣ استخراج ارزهای ترند از CoinGecko (حدود 15 تا 20 ارز)
...
def main():
    ...
    print(f"مجموعاً {len(symbols)} ارز برای اسکن آماده شد.")

    for tf in TIMEFRAMES:
        for sym in symbols:
            analyze_rsi_cycle(sym, tf)
            time.sleep(0.06)  # مکث بهینه برای فرار از Rate Limit بایننس
