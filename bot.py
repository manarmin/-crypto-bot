...
def main():
  ...
  send_telegram(
      '🚀 *ربات مجهز به استراحت ۳۰ دقیقه‌ای فعال شد!*\n💎 بررسی دقیق ۱۰۰ ارز'
      ' برتر کاملاً هماهنگ با کندل‌های ۳۰ دقیقه‌ای.'
  )
  ...
  while True:
    ...
    for tf in TIMEFRAMES:
      for sym in symbols:
        ...
        time.sleep(
            1.5
        )  # مکث ۱.۵ ثانیه‌ای بین هر ارز برای تخلیه کامل پردازنده
    gc.collect()
    time.sleep(
        1800
    )  # ۳۰ دقیقه استراحت مطلق بین دورها (دقیقاً هماهنگ با کندل 30m)
