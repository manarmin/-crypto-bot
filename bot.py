...
TOP_LIMIT = 150  # سقف بهینه برای جلوگیری از Rate Limit بایننس
    ...
    for full_url in urls:
        try:
            ...
            if e.code in [429, 418]:
                print("⚠️ محدودیتی از طرف بایننس رخ داد، ۵ ثانیه صبر میکنیم...")
                time.sleep(5)
            continue
        except Exception:
            continue
            ...
            for item in paprika_data[:220]:
                ...
                if pair in binance_usdt_pairs and pair not in ranked_symbols:
                    ...
                    if len(ranked_symbols) == TOP_LIMIT:
                        print(f"✅ {TOP_LIMIT} ارز برتر مارکت‌کپ استخراج شد.")
                        return ranked_symbols
    except Exception as e:
        print(f"هشدار لایه ۱: {e}")

    # لایه ۲ پشتیبان: پرحجم‌ترین‌های بایننس
