...
# لیست رزرو بر پایه BTC در صورت قطعی کامل API ها
STATIC_FALLBACK = [
    'ETHBTC', 'SOLBTC', 'BNBBTC', 'XRPBTC', 'DOGEBTC', 'ADABTC',
    'AVAXBTC', 'SHIBBTC', 'DOTBTC', 'LINKBTC', 'BCHBTC', 'NEARBTC', 'SUIBTC',
    'LTCBTC', 'UNIBTC', 'PEPEBTC', 'APTBTC', 'ICPBTC', 'ETCBTC', 'XMRBTC'
]
...
def get_target_symbols():
    """استخراج ارزها بر پایه BTC به همراه دسته‌بندی آن‌ها (ترند یا پرحجم)"""
    target_symbols = {}
    binance_btc_pairs = {}

    # 1️⃣ دریافت کل مارکت بایننس برای فیلتر حجم جفت‌های BTC
    ticker_data = fetch_json('/api/v3/ticker/24hr')
    if ticker_data:
        for item in ticker_data:
            sym = item.get('symbol', '')
            # فقط جفت‌ارزهایی که با BTC تمام می‌شوند
            if sym.endswith('BTC'):
                base = sym.replace('BTC', '')
                if base not in EXCLUDED_BASE_ASSETS and not is_leveraged_token(sym):
                    try:
                        vol = float(item.get('quoteVolume', 0)) # حجم بر اساس BTC
                        binance_btc_pairs[sym] = vol
                    except ValueError:
                        continue

    # اگر بایننس کلا قطع بود از لیست رزرو استفاده کن
    if not binance_btc_pairs:
        print("⚠️ بایننس در دسترس نیست، استفاده از لیست رزرو (BTC).")
    ...
    try:
        ...
        if cg_data and 'coins' in cg_data:
            for item in cg_data['coins'][:20]:
                ...
                pair = f"{cg_sym}BTC"
                # بررسی می‌کنیم که این ارز حتماً در بایننس با جفت BTC لیست شده باشد
                if pair in binance_btc_pairs and pair not in target_symbols:
                    target_symbols[pair] = '🔥 ترند بازار (CoinGecko)'
                    trending_count += 1
    except Exception as e:
        print(f"خطا در دریافت CoinGecko: {e}")

    print(f"✅ تعداد {trending_count} ارز ترند (جفت BTC) شناسایی و اضافه شد.")

    # 3️⃣ استخراج 200 ارز اول بایننس از نظر حجم معاملات بر پایه BTC
    sorted_by_volume = sorted(binance_btc_pairs.items(), key=lambda x: x[1], reverse=True)
    ...
    print(f"✅ تعداد {volume_count} ارز پرحجم بایننس (جفت BTC) اضافه شد.")
    ...
    try:
        ...
        if latest_signal:
            ...
            if last_alerted.get(key) != alert_id:
                ...
                # پاک کردن کلمه BTC از انتهای نماد برای نمایش بهتر در تلگرام (مثلا نمایش ETH بجای ETHBTC)
                clean_symbol = symbol.replace('BTC', '')
                msg = (
                    f"*{latest_signal['type']}*\n\n"
                    f'📌 *ارز:* `{clean_symbol}/BTC`\n'
                    f'🏷 *گروه:* `{category}`\n'
                    f'⏱ *تایم‌فریم:* `{tf}`\n'
                    f"💵 *قیمت:* `{latest_signal['price']:.8f}` ₿\n"
...
def main():
    print(f"شروع بررسی بازار جفت‌ارزهای BTC...")
