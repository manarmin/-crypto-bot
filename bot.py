...
# توکن ربات اصلی (برای PythonAnywhere)
MAIN_BOT_TOKEN = '8027946799:AAGhMQGDcEkMnH8PYClOWFMNKbEOLs_0PyY'

# توکن ربات تست جدید (@Manarminbot)
TEST_BOT_TOKEN = '8778525679:AAF0DG2sZLkuega7VJpOg5KQdpteAoA66NU'

# پیش‌فرض روی ربات تست تنظیم شده تا در گیت‌هاب حتماً به Manarminbot پیام بفرستد
...
def get_top_100_market_cap_symbols():
    ...
    if not ticker_data:
        return ALWAYS_INCLUDE + ['ETHUSDT', 'SOLUSDT', 'JUPUSDT']
    ...
    if coingecko_data and isinstance(coingecko_data, list):
        for coin in coingecko_data:
            ...
            if symbol in binance_usdt_pairs and symbol not in EXCLUDED_BASE_ASSETS:
                pair = f'{symbol}USDT'
    ...
    if len(ranked_symbols) < 50:
        coincap_data = fetch_json('https://api.coincap.io/v2/assets?limit=200')
        if coincap_data and 'data' in coincap_data:
            for item in coincap_data['data']:
                symbol = item.get('symbol', '').upper()
                if (
                    symbol in binance_usdt_pairs
                    and symbol not in EXCLUDED_BASE_ASSETS
                ):
                    pair = f'{symbol}USDT'
                    if pair not in ranked_symbols:
                        ranked_symbols.append(pair)
...
def main():
    ...
    # ارسال پیام تست فوری به تلگرام
    send_telegram("🚀 *ربات تست فعال شد!* \nبررسی ۱۰۰ ارز برتر بازار آغاز گردید...")

    symbols = get_top_100_market_cap_symbols()
    print(f"تعداد {len(symbols)} ارز برتر شناسایی شد.")
    ...
    gc.collect()
    print("بررسی کامل شد و پیام‌ها در صورت وجود سیگنال ارسال شدند.")
