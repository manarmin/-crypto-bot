...
def get_top_100_market_cap_symbols():
    # دریافت اطلاعات ۲۴ ساعته تمام جفت‌ارزهای بایننس
    ...
    for item in ticker_data:
        ...
        if sym.endswith('USDT'):
            ...
            # حذف استیبل‌کوین‌ها و توکن‌های اهرم‌دار
            if base not in EXCLUDED_BASE_ASSETS and not is_leveraged_token(sym):
                try:
                    # حجم دلاری ۲۴ ساعت گذشته (quoteVolume)
                    quote_volume = float(item.get('quoteVolume', 0))
                    valid_pairs.append((sym, quote_volume))
                except ValueError:
                    continue

    # مرتب‌سازی بر اساس بیشترین حجم معاملات ۲۴ ساعت گذشته
    valid_pairs.sort(key=lambda x: x[1], reverse=True)

    ranked_symbols = []
    
    # اولویت اول: ارزهای اجباری تعیین شده مثل BTCUSDT
    for forced_sym in ALWAYS_INCLUDE:
        if forced_sym not in ranked_symbols:
            ranked_symbols.append(forced_sym)

    # افزودن پرحجم‌ترین ارزهای بایننس تا سقف TOP_LIMIT (۱۰۰ ارز)
